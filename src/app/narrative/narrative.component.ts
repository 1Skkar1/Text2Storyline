...
export class NarrativeComponent implements OnChanges {
  ...
  ngOnInit() {
    ...
    else if (this.queryType == 'freetext') {
  ...
  public updateFreeText() {
    if (this.options.result) {
      let last = "";
      this.numTotal = this.options.result.TempExpressions.length;
      this.numTotal2 = this.options.result.TempExpressions.filter((cada) => {
        let f1 = this.options.result.Score[cada[0].toLowerCase()];
        return f1[Object.keys(this.options.result.Score[cada[0].toLowerCase()])[0]][0] > 0.35;
      }).length;
      last = "";
      this.differentValues = this.options.result.TempExpressions.sort(
        (a, b) => a[0] - b[0]
      ...
      if (this.options.score == "doc") {
        this.differentRelValues = this.differentValues.filter(
          (element, index, array) => {
            const a = element[0].toLowerCase() + "";
            let f1 = this.options.result.Score[a];
            return f1[Object.keys(this.options.result.Score[a])[0]][0] > 0.35;
          }
        );
      }
      else {
      ...
      for (const i in Object.keys(this.options.result.Score)) {
        if (this.options.score == "doc") {
          let value_to_be_replaced = Object.keys(this.options.result.Score)[i];
          let value_to_replace_for = this.options.result.TempExpressions.filter((a) => {
            return a[0].toLowerCase() == Object.keys(this.options.result.Score)[i];
          })[0][1];
          value_to_replace_for =
            "<strong><d>" + value_to_replace_for + "</d></strong>";
          let sentence_to_write = this.options.result.SentencesNormalized.map((a) => {
            if (
              a
                .toLowerCase()
                .toString()
                .search(Object.keys(this.options.result.Score)[i].toLowerCase()) != -1
            ) {
              let nova = a.replace(
                "<d>" + value_to_be_replaced + "</d>",
                "<d>" + value_to_replace_for + "</d>"
          ...
          let f1 = this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()];
          if (f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] >= 0.9) {
            a =
              '<p class="noticem8">Score: ' +
              f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] +
              "</p><p>" +
              sentence_to_write +
              "</p>";
          ...
          else if (f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] >= 0.7) {
            a =
              '<p class="noticem4">Score: ' +
              f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] +
              "</p><p>" +
          ...
          else if (f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] >= 0.5) {
            a =
              '<p class="noticem6">Score: ' +
              f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] +
              "</p><p>" +
          ...
          else if (f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] >= 0.35) {
            a =
              '<p class="noticem5">Score: ' +
              f1[Object.keys(this.options.result.Score[Object.keys(this.options.result.Score)[i].toLowerCase()])[0]][0] +
              "</p><p>" +
          ...
          for (const xd in this.options.result.Score[Object.keys(this.options.result.Score)[i]]) {
            ...
            d.push({
              x: Object.keys(this.options.result.Score)[i],
              y: this.options.result.Score[Object.keys(this.options.result.Score)[i]][xd][0],
              series: xd,
            });

            let sentence_to_write = this.options.result.SentencesNormalized[xd.toString()]
              .split('"')
              .join("''");
            let data_chave = Object.keys(this.options.result.Score)[i];

            let data_chave_replaced_by =
              "<strong>" + this.options.result.Score[data_chave][xd][1][0] + "</strong>";
            sentence_to_write = sentence_to_write.replace(
              data_chave,
              data_chave_replaced_by
            );
            ...
            // tslint:disable-next-line: whitespace
            // tslint:disable-next-line: max-line-length
            if (this.options.result.Score[Object.keys(this.options.result.Score)[i]][xd][0] >= 0.9) {
              // tslint:disable-next-line: whitespace
              // tslint:disable-next-line: max-line-length
              valorDeA +=
                '<p class="noticem8">Score: ' +
                this.options.result.Score[Object.keys(this.options.result.Score)[i]][xd][0] +
                "</p><p>" +
                sentence_to_write +
                "</p>";
            ...
            else if (this.options.result.Score[Object.keys(this.options.result.Score)[i]][xd][0] >= 0.7) {
              // tslint:disable-next-line: whitespace
              // tslint:disable-next-line: max-line-length
              valorDeA +=
                '<p class="noticem4">Score: ' +
            ...
            else if (this.options.result.Score[Object.keys(this.options.result.Score)[i]][xd][0] >= 0.5) {
              // tslint:disable-next-line: whitespace
              // tslint:disable-next-line: max-line-length
              valorDeA +=
                '<p class="noticem6">Score: ' +
            ...
            else if (this.options.result.Score[Object.keys(this.options.result.Score)[i]][xd][0] >= 0.35) {
              // tslint:disable-next-line: whitespace
              // tslint:disable-next-line: max-line-length
              valorDeA +=
                '<p class="noticem5">Score: ' +
        ...
        /^\d+$/.test(
          Object.keys(this.options.result.Score)[i].substring(0, 10).split("-").join("")
        )
          ? ""
          : c.pop();
        /^\d+$/.test(
        ...
        for (let j = 0; j < this.topTen[i].date.length; j++) {
          if (this.topTen[i].dateOG[j].length > 4) {
            if (this.topTen[i].text.includes(this.topTen[i].dateOG[j])) {
              ...
              let plusInfo = "<a class='naoBranco' target=\"_blank\" href='" + this.topTen[i].url + "'><b>[+]</b></a>"
