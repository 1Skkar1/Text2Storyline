...
@Component({
  selector: "app-annotatedtext",
  templateUrl: "./annotatedtext.component.html",
  styleUrls: ["./annotatedtext.component.css"],
})

export class AnnotatedtextComponent implements OnInit, OnChanges {
  ...
  displayEntities() {
    ...
    for (let i = 0; i < this.wikiData.length; i++) {
      if (!this.text.includes(">" + this.wikiData[i].title + "<")
        ...
        if (this.wikiData[i].class == 'human' || this.wikiData[i].class == 'person' || this.wikiData[i].class == 'organization' || this.wikiData[i].class == 'sovereign state' || this.wikiData[i].class == 'country' || this.wikiData[i].class == 'cityp') {
          ...
          this.text = this.text.replace(new RegExp(this.wikiData[i].title,'gi'), "<a class='darkblue' target='_blank' href='" + this.wikiData[i].url + "'><b>" + this.wikiData[i].title.toLowerCase() + "</b></a>")
        }
      }
    }
